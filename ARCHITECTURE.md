...
**The Oracle VM.** `ocr-service`, `transcription-relay` and `law-monitor` run on a small ARM box outside Supabase — see [docs/oracle-vm.md](docs/oracle-vm.md) for access, the port map, and how to add a service to it.
...
- **The UI says "Project"; the code and database say "matter".** The firm asked for the word Project everywhere an associate reads it (2026-09-07), and that was done as a display-string rename only. Tables (`matters`, `matter_documents`, `matter_relevant_laws`, …), routes (`/matters/:id`), hooks (`useMatter*`), props (`matterId`) and the storage bucket (`matter-documents`) all still say matter, deliberately: renaming those touches RLS policies, edge functions, the mandate bot's sync and every deep link, for no user-visible gain. So a "Project" in the UI is a row in `matters`. When adding UI text, write Project; when adding code, keep matter. The two legacy hooks `useMatterChat.ts` and `useDrafting.ts` are dead (superseded by `useChat.ts`) and were left alone.
- **A long draft is written across several invocations of `chat`, not one.** Supabase kills an edge function at about 150s of wall clock with no chance to run cleanup — measured, a draft turn streamed ~24k characters and was then cut off with no `done` event and nothing saved at all. So generation stops itself at `GENERATION_BUDGET_MS` (115s), saves the partial reply with `metadata.incomplete`, and reports the turn unfinished; the client immediately calls back with `continueMessageId`, and a fresh invocation resumes from exactly that character by passing the partial back as a trailing assistant turn. The document is only turned into an `ai_artifacts` row once it is actually finished, so a partial reply holds raw `<artifact …>` markup — `MarkdownMessage` hides an unterminated block behind a "Writing document…" placeholder. Raising `max_tokens` alone does **not** fix truncation here; the wall clock binds long before the token cap.
- **Edit mode works on a copy, never the file.** Picking a version (`EditDocumentDialog`) extracts its text through `extract-document-text`, starts a thread with `skill = {key: "edit", documentVersionId, matterDocumentId}`, and stores the document as that thread's first `ai_artifacts` row (`data.editSource`, `data.original = true`). Each turn the `chat` function takes the **newest draft artifact in the thread** as the base — so edits are iterative — and puts its full text in the prompt as CURRENT DOCUMENT. *Save as new version* files the result as the next `document_versions` row of the same `matter_documents` row via `editSource.matterDocumentId`; until then nothing on the project changes.
- **Documents mentioned by name are attached automatically.** `resolveReferences` in the `chat` function matches a message against the project's document titles / filenames / a document type the project has exactly one of, honours "v2" / "version 2", and attaches that version's full text for the turn (it also lands on the user message as a chip). The prompt carries a DOCUMENTS ON THIS PROJECT list so the model knows what exists and asks rather than guesses.
- **A half-written reply is resumable, not finalised.** The client's `useSendChatMessage.resume()` re-sends `continueMessageId` for a message whose `metadata.incomplete` is set; the list shows *Continue writing* on it. Pressing Stop aborts the upstream Anthropic stream (`req.signal` → `clientSignal`) and what was written is kept as a finished reply with `metadata.stopped`.
